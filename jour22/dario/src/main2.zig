...
pub fn main() !void {
    ...
    while (try stdin.readUntilDelimiterOrEofAlloc(allocator, '\n', 16) orelse null) |line| {
        ...
        block.fallen = false;
        block.blocks_that_would_fall = 0;
    ...
    var total_blocks_that_would_fall : u32= 0;
    for (blocks.items) |*block| {
        total_blocks_that_would_fall += block.blocks_that_would_fall;
    }

    std.debug.print("The amount of blocks that can be removed is {}\n", .{total_blocks_that_would_fall});
}
...
fn fall_block(block: *Block, columns: Columns) void {
    ...
    if (only_one_supporting) |supporting| {
        supporting.blocks_that_would_fall += 1;
    }
